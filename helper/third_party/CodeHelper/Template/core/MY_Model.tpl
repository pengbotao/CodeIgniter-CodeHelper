...
class MY_Model extends CI_Model 
{
     ...
     * 更新表记录
     * 
     * @param array $attributes 需要更新的数组，如需对值不转义，可以传递数组，如：updateAll(array('price' => array('price + 1', false)), array('id > ' => 2)))
     * @param array $where
     ...
     * 执行SQL，返回影响的行数
     * 
     * @param string $sql 执行语句，如 UPDATE `test` SET `stat` = 3 WHERE WHERE id = ?
     * @param array $param array(3)
     * @return boolean|int 执行成功返回影响行数， 执行失败返回false，若执行成功但影响行数为0 返回true
     */
